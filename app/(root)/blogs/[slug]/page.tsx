...
function Blogs() {
  ...
  const ContentSection = (blogContent: IBlogContent[]) => {
    return blogContent.map((content, index) => (
      <div
        key={index}
        className="mb-1 flex flex-col align-center items-center text-left"
      >
        <div className="contentSection my-10 w-3/4">
          <h3 className="text-5xl font-semibold mb-6 mt-2 text-red-800">
            {content.heading}
          </h3>
          ...
          {content.paragraphs.map((para, idx) => (
            <p key={idx} className="mb-2 text-theme-primary-dark">
              {para}
            </p>
          ))}
        </div>
